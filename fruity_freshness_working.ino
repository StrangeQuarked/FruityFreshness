...
float fruitQuantityWeights[] = {0.05, 0.175, 0.05, 0.05};//apples aren't very methane-y, 
...
void checkSensorAndAlert() {
  ...
  float base_limit = 200;
  float calced_limit = 0;
  float total_modifier = 0;


  for (int i = 0; i < sizeof(menuItems) / sizeof(menuItems[0]); i++) {
    total_modifier += fruitQuantities[i] * fruitQuantityWeights[i];
  }
  calced_limit = base_limit * (1 + total_modifier);//use the number of fruits to linearly (i hope its actually linear) scale the methane threshold needed
  Serial.print("THEORETICAL LIMIT: ");
  Serial.print(calced_limit);
  Serial.println();
  
  if (sensorValue > calced_limit) {
